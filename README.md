...
BQuant is an LLM Skill that detects and signals on the price-discovery gap between bStocks
(tokenized US equities trading 24/7 on BNB Chain) and their reference market NYSE/Nasdaq —
which is closed on weekends.**,
**Track 2: Strategy Skills**.

Every signal the skill produces is logged on-chain, so the claim "this signal actually ran" is
checkable by anyone.
...
market to correct them. BQuant measures that drift in real time and signals a convergence
trade anticipating reversion once the real market reopens.

This gap could not have existed before bStocks launched.
...
**Why self-logged data BQuant builds its own historical record: it polls live
prices on a fixed interval and logs each snapshot itself, rather than querying for a past that
isn't available to it. Every number in this repo is either a live API response or a confirmed
on-chain transaction. Nothing is backfilled or simulated.
...
```
              ...
              data/weekend_snapshots.json
                            ...
                            ▼
                   drift_engine.py
              calculate_drift() / generate_signal()
                            │
                            ▼
              BQuantSignalRegistry.sol (BSC testnet)
                      logSignal()
                            ...
                            ▼
                   website/ dashboard
              (live drift, price history, on-chain log)
```
